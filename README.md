...
KeepOn uses device and Flutter plugin APIs to enhance the application beyond a static task list:

- **Local notification service** through `flutter_local_notifications` to schedule repeated task reminders on the device.
- **Timezone service** through `timezone` so scheduled reminders can be calculated consistently.
- **Device sensor API** through `sensors_plus` to read accelerometer data and make reminder timing aware of whether the phone has been stationary.
- **Local persistence services** through Hive and SharedPreferences to store tasks, notification settings, and quiet-hour preferences between app launches.
...
**Landing page:** [Open index.html](index.html)
...
I confirm that this project is my own work for the UCL CASA0015 Mobile Systems & Interactions coursework. Any external libraries, frameworks, tools, or assets used in the project are acknowledged through the dependency list, source files, and project documentation.
